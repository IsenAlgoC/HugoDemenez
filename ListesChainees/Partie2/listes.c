	...
	if (CurrentElement != NULL) {
		...
		if (Liste->size <= position) { // insertion en queue
			...
			if (NewElement != NULL) {
				...
				//Le nouvel element devient la queue
				Liste->tail = NewElement;
				//Le nouvel element pointe vers NULL
				NewElement->next = NULL;
				//La taille de la liste augmente de 1
				Liste->size++;
				return(1);
			}
			else {
...
int DisplayList(LinkedList* Liste){
	if (Liste->head == NULL || Liste->size==0) {
		...
		if (elem->next == NULL) {
			...
			//Sinon on affiche tous les elements jusqu'à arriver au dernier
			while (elem != NULL) {
				printf("%d", elem->data);
				elem = elem->next;
			}
...
int InitRandLinkedList(LinkedList* List, int ListSize) {
	if (List == NULL) {
		...
		SingleLinkedListElem *randomelement =	NewLinkedListElement(rangedRand(0,9));
